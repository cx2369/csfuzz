...
bool initialized_cfg_distance(graph_cfg_t &cfg, std::string prev_path, graph_cfg_t &new_cfg)
{
    ...
    bo::dijkstra_shortest_paths(new_cfg, fst_node, boost::predecessor_map(&preds[0]).distance_map(boost::make_iterator_property_map(distances.begin(), idmap)).weight_map(boost::get(&Edge_cfg::edge_weight, new_cfg)));
        ...
        for (auto edge : make_iterator_range(bo::out_edges(src, cfg_graph_llvm)))
        {
            ...
            if (cfg_graph_llvm[dst].label.find("%") != std::string::npos)
            {
                ...
                for (auto new_edge : make_iterator_range(bo::out_edges(dst, cfg_graph_llvm)))
                {
                    ...
                    if (cfg_graph_llvm[new_dst].label.find("%") != std::string::npos)
                        continue;
...
{
    for (auto src : boost::make_iterator_range(vertices(cfg)))
        if (cfg[src].is_fstbb == true)
            return cfg[src].node_id;
    return 0;
}
...
bool initialized_cg_distance(graph_cg_t &cg, std::string &prev_path, graph_cg_t &new_cg)
{
    ...
    bo::dijkstra_shortest_paths(new_cg, fst_node, boost::predecessor_map(&preds[0]).distance_map(boost::make_iterator_property_map(distances.begin(), idmap)).weight_map(bo::get(&Edge_cg::edge_weight, new_cg)));
...
bool construct_cg(std::string cgname, graph_cg_t &cg_graph, graph_cg_t &new_cg)
{
    ...
    for (auto src : boost::make_iterator_range(vertices(cg_graph_llvm)))
    {
        for (auto edge : make_iterator_range(bo::out_edges(src, cg_graph_llvm)))
        {
            ...
            tem++;
            double cur = static_cast<double>(tem) / total * 100.0;
            if (static_cast<int>(cur) - static_cast<int>(prev) >= 1)
            {
                prev = cur;
                std::cout << __LINE__ << "-process:" << static_cast<int>(cur) << "%" << std::endl;
                std::cout << "\n" << std::flush;
            }
        }
    }
...
bool calc_and_save_map(std::string temporary_dir, graph_cg_t &cg_graph)
{
    ...
    for (auto vd_start : boost::make_iterator_range(vertices(cg_graph)))
    {
        ...
        bo::dijkstra_shortest_paths(cg_graph, vd_start, boost::predecessor_map(&preds[0]).distance_map(boost::make_iterator_property_map(distances.begin(), idmap)).weight_map(bo::get(&Edge_cg::edge_weight, cg_graph)));
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        desc.add_options()("help,h", "produce help message")("dot,d", po::value<std::string>()->required(), "Path to dot-file representing the "
                                                                                                            "control flow graph.")("temp,t", po::value<std::string>()->required(), "Temporary folder contain dot "
                                                                                                                                                                                   "files.")("out,o", po::value<std::string>()->required(), "Path to out-file contain dot files "
                                                                                                                                                                                                                                            "with initial distance.")("outmap,a", po::value<std::string>(), "Path to out-file contain shortest "
                                                                                                                                                                                                                                                                                                            "function distance.")("mode,m", po::value<std::string>()->required(), "Run in cfg mode or cg mode.");
